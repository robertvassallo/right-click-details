...
return {
	en = {
		...
		mod_desc = "Right-click a town, industry, station or depot for a compact readout, "
			...
			.. "capacity, then a row per commodity with its line where that can be "
			.. "told for certain. Inbound and outbound throughput sit underneath.\n\n"
			...
			.. "waiting at. The \"waiting here\" total IS exact for the stop, so the "
			.. "two will not add up to each other.\n\n"
			.. "Freight amounts are exact. The line beside a commodity is a GOOD "
			.. "GUESS, not a fact - where two or more fit, the row reads \"2 lines\" "
			.. "and each is listed underneath in its own colour. The game never "
			.. "records which line an individual waiting crate is queued for, so the "
			.. "panel works it out from what each line\'s wagons are set up to haul. "
			.. "Wagons left on \"auto\" report only what they are allotted to right "
			.. "now, so a line that carries a commodity daily can go unnamed for it, "
			.. "and a row can read differently a minute later. The amounts never "
			.. "change - only the line beside them.\n\n"
			.. "A station serving more than ten lines shows the ten busiest. All of "
			.. "them are still read, so the commodity rows are unaffected - only the "
			.. "list is trimmed, because the panel is meant to be a glance rather "
			.. "than a second window.\n\n"
			.. "Produced, Shipped and Used on an industry, and Outbound and Inbound on "
			.. "a station, are lifetime totals since it was built - not per-year rates. "
			.. "The game's per-period counters exist but are never filled in, so a rate "
